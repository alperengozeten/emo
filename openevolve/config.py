...
from typing import TYPE_CHECKING, Any, Callable, Dict, List, Optional, Union, get_args, get_origin
...
def _fallback_convert_value(type_hint: Any, value: Any) -> Any:
    """Best-effort dataclass construction when dacite is unavailable."""
    if value is None:
        return None
    ...
    if origin in (list, List):
        ...
        return [_fallback_convert_value(inner_type, item) for item in value]

    if origin in (dict, Dict):
        value_type = args[1] if len(args) >= 2 else Any
        return {key: _fallback_convert_value(value_type, item) for key, item in value.items()}

    if origin is Union:
        non_none_args = [arg for arg in args if arg is not type(None)]
        if not non_none_args:
            return value
        for candidate_type in non_none_args:
            try:
                return _fallback_convert_value(candidate_type, value)
            except Exception:
                continue
        return value
    ...
    if isinstance(type_hint, type) and is_dataclass(type_hint):
        return _fallback_from_dict(type_hint, value)
...
def _fallback_from_dict(data_class: Any, data: Dict[str, Any]) -> Any:
    """Construct a nested dataclass without dacite."""
    ...
    for field_info in fields(data_class):
        if field_info.name not in data:
            continue
        kwargs[field_info.name] = _fallback_convert_value(field_info.type, data[field_info.name])
...
@dataclass
class WandbConfig:
    """Optional Weights & Biases tracking configuration."""
    ...
    log_best_program_artifact: bool = True
...
class Config:
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> "Config":
        ...
        if dacite is not None:
            ...
            config = _fallback_from_dict(cls, config_dict)
