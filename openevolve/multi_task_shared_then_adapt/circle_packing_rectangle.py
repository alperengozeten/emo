"""Canonical rectangle circle-packing task family definitions for EMO-STA."""
...
CIRCLE_PACKING_RECTANGLE_TASK_SELECTOR_ENV_VAR = "CIRCLE_PACKING_RECTANGLE_TASK_ID"
CIRCLE_PACKING_RECTANGLE_SHARED_SELECTOR = "all"
DEFAULT_STAGE1_TIMEOUT_SECONDS = 60
DEFAULT_FULL_TIMEOUT_SECONDS = 180

CIRCLE_PACKING_RECTANGLE_METRIC_KEYS: tuple[str, ...] = (
    "sum_radii",
    "target_sum_radii",
    "target_ratio",
    "validity",
...
class CirclePackingRectangleTaskSpec:
    """Stable task identity for the rectangle circle-packing EMO-STA family."""
    ...
    container_name: str = "perimeter4_rectangle"
...
CIRCLE_PACKING_RECTANGLE_TRAIN_TASK_SPECS: tuple[CirclePackingRectangleTaskSpec, ...] = (
    CirclePackingRectangleTaskSpec(
        task_id="cp_rect_n20",
        task_index=0,
        display_name="CirclePackingRectangleN20",
        n_circles=20,
        target_sum_radii=2.305,
    ),
    CirclePackingRectangleTaskSpec(
        task_id="cp_rect_n21",
        task_index=1,
        display_name="CirclePackingRectangleN21",
        n_circles=21,
        target_sum_radii=2.365,
    ),
    CirclePackingRectangleTaskSpec(
        task_id="cp_rect_n22",
        task_index=2,
        display_name="CirclePackingRectangleN22",
        n_circles=22,
        target_sum_radii=2.425,
    ),
    CirclePackingRectangleTaskSpec(
        task_id="cp_rect_n23",
        task_index=3,
        display_name="CirclePackingRectangleN23",
        n_circles=23,
        target_sum_radii=2.484,
    ),
)

# Backwards-compatible name for the in-distribution training task set.
CIRCLE_PACKING_RECTANGLE_TASK_SPECS = CIRCLE_PACKING_RECTANGLE_TRAIN_TASK_SPECS

CIRCLE_PACKING_RECTANGLE_TASKS_BY_ID: Dict[str, CirclePackingRectangleTaskSpec] = {
    task.task_id: task for task in CIRCLE_PACKING_RECTANGLE_TASK_SPECS
}

CIRCLE_PACKING_RECTANGLE_OOD_TASK_SPECS: tuple[CirclePackingRectangleTaskSpec, ...] = (
    CirclePackingRectangleTaskSpec(
        task_id="cp_rect_n19",
        task_index=4,
        display_name="CirclePackingRectangleN19",
        n_circles=19,
        target_sum_radii=2.241,
    ),
    CirclePackingRectangleTaskSpec(
        task_id="cp_rect_n24",
        task_index=5,
        display_name="CirclePackingRectangleN24",
        n_circles=24,
        target_sum_radii=2.535,
    ),
)

CIRCLE_PACKING_RECTANGLE_ADDITIONAL_OOD_TASK_SPECS: tuple[
    CirclePackingRectangleTaskSpec, ...
] = (
    CirclePackingRectangleTaskSpec(
        task_id="cp_rect_n25",
        task_index=6,
        display_name="CirclePackingRectangleN25",
        n_circles=25,
        target_sum_radii=2.592,
    ),
)

CIRCLE_PACKING_RECTANGLE_ALL_OOD_TASK_SPECS: tuple[
    CirclePackingRectangleTaskSpec, ...
] = (
    *CIRCLE_PACKING_RECTANGLE_OOD_TASK_SPECS,
    *CIRCLE_PACKING_RECTANGLE_ADDITIONAL_OOD_TASK_SPECS,
)

CIRCLE_PACKING_RECTANGLE_OOD_TASKS_BY_ID: Dict[str, CirclePackingRectangleTaskSpec] = {
    task.task_id: task for task in CIRCLE_PACKING_RECTANGLE_ALL_OOD_TASK_SPECS
}

CIRCLE_PACKING_RECTANGLE_ALL_EVAL_TASK_SPECS: tuple[
    CirclePackingRectangleTaskSpec, ...
] = (
    *CIRCLE_PACKING_RECTANGLE_TRAIN_TASK_SPECS,
    *CIRCLE_PACKING_RECTANGLE_ALL_OOD_TASK_SPECS,
)

CIRCLE_PACKING_RECTANGLE_ALL_EVAL_TASKS_BY_ID: Dict[
    str, CirclePackingRectangleTaskSpec
] = {
    task.task_id: task for task in CIRCLE_PACKING_RECTANGLE_ALL_EVAL_TASK_SPECS
}


def resolve_task_specs(selector: Optional[str]) -> List[CirclePackingRectangleTaskSpec]:
    """Resolve a task selector into concrete rectangle circle-packing tasks."""
    normalized = (selector or CIRCLE_PACKING_RECTANGLE_SHARED_SELECTOR).strip()
    if not normalized or normalized == CIRCLE_PACKING_RECTANGLE_SHARED_SELECTOR:
        return list(CIRCLE_PACKING_RECTANGLE_TASK_SPECS)
    if normalized not in CIRCLE_PACKING_RECTANGLE_TASKS_BY_ID:
        available = ", ".join(task.task_id for task in CIRCLE_PACKING_RECTANGLE_TASK_SPECS)
        raise ValueError(
            f"Unknown rectangle circle-packing task '{normalized}'. Available: {available}"
        )
    return [CIRCLE_PACKING_RECTANGLE_TASKS_BY_ID[normalized]]


def resolve_eval_task_specs(selector: Optional[str]) -> List[CirclePackingRectangleTaskSpec]:
    ...
    normalized = (selector or CIRCLE_PACKING_RECTANGLE_SHARED_SELECTOR).strip()
    if not normalized or normalized == CIRCLE_PACKING_RECTANGLE_SHARED_SELECTOR:
        return list(CIRCLE_PACKING_RECTANGLE_TRAIN_TASK_SPECS)
    if normalized not in CIRCLE_PACKING_RECTANGLE_ALL_EVAL_TASKS_BY_ID:
        available = ", ".join(
            task.task_id for task in CIRCLE_PACKING_RECTANGLE_ALL_EVAL_TASK_SPECS
        )
        raise ValueError(
            f"Unknown rectangle circle-packing evaluation task '{normalized}'. "
            f"Available: {available}"
        )
    return [CIRCLE_PACKING_RECTANGLE_ALL_EVAL_TASKS_BY_ID[normalized]]


def resolve_ood_task_specs(selector: Optional[str]) -> List[CirclePackingRectangleTaskSpec]:
    """Resolve post-hoc OOD task selectors for rectangle circle packing."""
    normalized = (selector or "all_ood").strip()
    if not normalized or normalized in {"all", "all_ood", "ood"}:
        return list(CIRCLE_PACKING_RECTANGLE_ALL_OOD_TASK_SPECS)

    requested_ids = [task_id.strip() for task_id in normalized.split(",") if task_id.strip()]
    if not requested_ids:
        return list(CIRCLE_PACKING_RECTANGLE_ALL_OOD_TASK_SPECS)

    resolved: List[CirclePackingRectangleTaskSpec] = []
    seen_task_ids: set[str] = set()
    for task_id in requested_ids:
        if task_id in CIRCLE_PACKING_RECTANGLE_TASKS_BY_ID:
            raise ValueError(
                f"Rectangle circle-packing task '{task_id}' is part of the seen "
                "training family, not the post-hoc OOD set"
            )
        task = CIRCLE_PACKING_RECTANGLE_OOD_TASKS_BY_ID.get(task_id)
        if task is None:
            available = ", ".join(
                task.task_id for task in CIRCLE_PACKING_RECTANGLE_ALL_OOD_TASK_SPECS
            )
            raise ValueError(
                f"Unknown rectangle circle-packing OOD task '{task_id}'. "
...
def normalize_task_metrics(
    ...
    *,
    ...
    """Normalize evaluator outputs into the stable rectangle metric schema."""
    ...
    validity_raw = _coerce_finite_float(raw_metrics.get("validity"), 0.0)
    alpha = _coerce_finite_float(raw_metrics.get("alpha"), float("nan"))
    alpha_valid = math.isfinite(alpha) and 0.0 < alpha <= 1.0
    height = 2.0 - alpha if alpha_valid else 0.0
    if not math.isfinite(height) or height <= 0.0:
        alpha_valid = False
        height = 0.0

    validity = 1.0 if validity_raw >= 0.5 and alpha_valid else 0.0
    if validity <= 0.0:
    ...
    aspect_ratio = alpha / height if height > 0.0 else 0.0
...
def aggregate_task_results(task_results: Iterable[Mapping[str, Any]]) -> Dict[str, float]:
    """Aggregate per-task rectangle results into one shared metric payload."""
    ...
    for key in CIRCLE_PACKING_RECTANGLE_METRIC_KEYS:
        aggregate[key] = sum(
...
def extract_task_result(artifacts: Mapping[str, Any], task_id: str) -> Optional[Dict[str, Any]]:
    ...
    for task_result in task_results:
        ...
        task = CIRCLE_PACKING_RECTANGLE_ALL_EVAL_TASKS_BY_ID.get(task_id)
        if task is None:
...
__all__ = [
    "CIRCLE_PACKING_RECTANGLE_ADDITIONAL_OOD_TASK_SPECS",
    "CIRCLE_PACKING_RECTANGLE_ALL_EVAL_TASK_SPECS",
    "CIRCLE_PACKING_RECTANGLE_ALL_EVAL_TASKS_BY_ID",
    "CIRCLE_PACKING_RECTANGLE_ALL_OOD_TASK_SPECS",
    "CIRCLE_PACKING_RECTANGLE_METRIC_KEYS",
    "CIRCLE_PACKING_RECTANGLE_OOD_TASK_SPECS",
    "CIRCLE_PACKING_RECTANGLE_OOD_TASKS_BY_ID",
